...
class FeffPathGroup(Group):
    ...
    def _pathparams(self, paramgroup=None, **kws):
        ...
        delarch.paramGroup._asteval('reff=%f' %self._feffdat.reff)
        ...
        out = []
        def k_n(x, z=1):  
            '''find the good string for paramGroup'''
            if delarch.paramGroup.has_key('%s_%d'%(x,z)): k_n(x,z+1)
            else: return '%s_%d'%(x,z)
        ...
        for param in ('degen', 's02', 'e0', 'ei',
            ...
            if isinstance(val, (str, unicode)):
                thispar = Parameter(name=k_n(param),  expr=val)
                delarch.paramGroup.add(thispar)
                #if isinstance(thispar, Parameter):
                #    thispar = thispar.value
                setattr(self, param, thispar)
                val = getattr(self, param).value
            elif isinstance(val, Parameter): 
                val = getattr(self, param).value
            out.append(val)                
            #    #print '\n'*3,val
            #    val=delarch.paramGroup._asteval(val)
            #out.append(val)
        return out
    ...
    def report(self):
        ...
        delarch.paramGroup._feffdat = self._feffdat
        delarch.paramGroup._asteval('reff= %g' %self._feffdat.reff)
