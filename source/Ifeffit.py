...
def set_environ():
    ...
    if os.name == 'nt':
        ...
        path_sep = ';'
        defaults = {'PGPLOT_DEV':'/GW',
                    'IFEFFIT_DIR':'C:\Program Files\Ifeffit'}
       

    pgdev = os.environ.get('PGPLOT_DEV',defaults['PGPLOT_DEV'])
    iffdir= os.environ.get('IFEFFIT_DIR',defaults['IFEFFIT_DIR'])
    iffbin = iffdir
    if os.name == 'nt': 
        iffbin= os.path.join(iffdir,'bin')
    os.environ['PGPLOT_DEV']  = pgdev
    os.environ['IFEFFIT_DIR'] = iffdir
    os.environ['IFEFFIT_BIN'] = iffbin
    os.environ['PGPLOT_DIR']  = iffbin
    os.environ['PGPLOT_FONT'] = os.path.join(iffbin,'grfont.dat')
    os.environ['PATH'] = "%s%s%s" % (iffbin,path_sep,os_path)
